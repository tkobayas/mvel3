...
public class EvaluatorBuilder<T, K, R> {
    ...
    public void addDeclaration(String name, Class clazz) {
        if (variableInfo == null) {
            variableInfo = ContextInfoBuilder.create(Type.type(Map.class));
        }
    ...
    public static class ContextInfoBuilder<T> {
        ...
        public ContextInfoBuilder<T> clone() {
            ContextInfoBuilder<T> cloned = new ContextInfoBuilder<>();
            cloned.declaration = this.declaration;
            cloned.vars = new ArrayList<>(this.vars);
            return cloned;
        }
